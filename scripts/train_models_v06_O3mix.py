...
def main():
    ...
    ap.add_argument('--train_opts', default='O0,O1,O2,O3')
    ...
    print(f"Train set (O3-mix): total={len(y)} pos={pos} neg={neg}")

    rf=RandomForestClassifier(n_estimators=600,max_depth=None,n_jobs=-1,random_state=42,class_weight='balanced_subsample')
    rf.fit(X,y)
    rf_loss=log_loss(y, rf.predict_proba(X))
    dump({'model': rf, 'feature_keys': keys}, os.path.join(args.out_dir,'start_detector_v06d_rf.joblib'))
    ...
    dump({'model': xgb, 'feature_keys': keys}, os.path.join(args.out_dir,'start_detector_v06d_xgb.joblib'))
