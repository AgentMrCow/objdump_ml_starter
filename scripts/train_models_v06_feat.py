    ...
    for opt in opts:
        for prog in programs:
            ...
            if os.path.exists(asm) and os.path.exists(label):
                pairs.append((asm,label,opt,prog))
    return pairs

def build_dataset(pairs, keys):
    samples={}
    for asm_path,label_path,opt,prog in pairs:
...
def main():
    ...
    ap.add_argument('--opts',default='O0,O1,O2')
    ...
    print(f"Train feat+bb: total={len(y)} pos={pos} neg={neg}")

    rf=RandomForestClassifier(n_estimators=800,max_depth=None,n_jobs=-1,random_state=42,class_weight='balanced_subsample')
    rf.fit(X,y)
    rf_loss=log_loss(y, rf.predict_proba(X))
    dump({'model': rf, 'feature_keys': keys}, os.path.join(args.out_dir,'start_detector_v06c_rf.joblib'))
    print(f"RF feat log-loss={rf_loss:.4f}")
    ...
    dump({'model': xgb, 'feature_keys': keys}, os.path.join(args.out_dir,'start_detector_v06c_xgb.joblib'))
    print(f"XGB feat log-loss={xgb_loss:.4f}")
